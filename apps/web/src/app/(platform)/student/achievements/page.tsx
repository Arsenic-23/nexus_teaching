...
const rarityConfig: Record<Rarity, { label: string; colors: string; glow: string; star: string }> = {
  COMMON: { label: 'Common', colors: 'border-white/10 bg-white/5 text-gray-300', glow: '', star: 'text-gray-400' },
  RARE: { label: 'Rare', colors: 'border-blue-500/30 bg-blue-500/10 text-blue-400', glow: 'shadow-[0_0_15px_rgba(59,130,246,0.15)]', star: 'text-blue-400' },
  EPIC: { label: 'Epic', colors: 'border-purple-500/30 bg-purple-500/10 text-purple-400', glow: 'shadow-[0_0_20px_rgba(168,85,247,0.2)]', star: 'text-purple-400' },
  LEGENDARY: { label: 'Legendary', colors: 'border-yellow-500/30 bg-yellow-500/10 text-yellow-400', glow: 'shadow-[0_0_25px_rgba(234,179,8,0.25)]', star: 'text-yellow-400' },
};
...
export default function AchievementsPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-wider">
          {(['COMMON', 'RARE', 'EPIC', 'LEGENDARY'] as Rarity[]).map((r) => {
            ...
            return (
              <span key={r} className={cn('px-3 py-1.5 rounded-lg border backdrop-blur-md', config.colors)}>
                {config.label}
              </span>
            );
          })}
      ...
      {/* Unlocked */}
      <div>
        <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-widest mb-4">Unlocked</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {unlocked.map((achievement) => {
            const config = rarityConfig[achievement.rarity];
            const Icon = achievement.icon;
            return (
              <Card key={achievement.id} className={cn('border backdrop-blur-2xl transition-all hover:scale-[1.03] hover:shadow-2xl cursor-pointer rounded-3xl overflow-hidden group', config.colors, config.glow)}>
                <CardContent className="p-6 text-center relative h-full flex flex-col justify-between">
                  <div className="absolute inset-0 bg-white/0 group-hover:bg-white/5 transition-colors pointer-events-none" />
                  <div>
                    <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-black/40 border border-white/10 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                    ...
                    <p className="font-bold text-sm text-white mb-1 leading-tight">{achievement.name}</p>
                    <p className="text-[11px] font-medium text-gray-400 mb-4 line-clamp-2">{achievement.description}</p>
                  </div>
                  <div className="mt-auto">
                    <div className="px-3 py-1.5 rounded-lg bg-black/40 border border-white/10 text-[10px] font-black text-white inline-block mb-2 uppercase tracking-wider">
                      +{achievement.xpReward} XP
                    </div>
                    <p className="text-[9px] font-bold text-gray-600 uppercase tracking-wider">{achievement.unlockedDate}</p>
                  </div>
                </CardContent>
              </Card>
            );
      ...
      {/* Locked */}
      <div>
        <h2 className="text-sm font-semibold text-gray-600 uppercase tracking-widest mb-4">Locked</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {locked.map((achievement) => {
            const config = rarityConfig[achievement.rarity];
            const Icon = achievement.icon;
            return (
              <Card key={achievement.id} className="border border-white/5 bg-black/20 backdrop-blur-md opacity-50 rounded-3xl overflow-hidden">
                <CardContent className="p-6 text-center h-full flex flex-col justify-between">
                  <div>
                    ...
                    <div className="flex items-center justify-center gap-1 mb-2">
                      ...
                      <span className={cn('text-[9px] font-black uppercase tracking-wider text-gray-600')}>
                        {config.label}
                      </span>
                    </div>
                    <p className="font-bold text-sm text-gray-400 mb-1 leading-tight">{achievement.name}</p>
                    <p className="text-[11px] font-medium text-gray-600 mb-4 line-clamp-2">{achievement.description}</p>
                  </div>
                  <div className="mt-auto">
                    <div className="px-3 py-1.5 rounded-lg bg-black/40 border border-white/5 text-[10px] font-black text-gray-500 inline-block uppercase tracking-wider">
                      +{achievement.xpReward} XP
                    </div>
                  </div>
                </CardContent>
