...
function LeaderboardEntry({ entry }: { entry: typeof weeklyData[0] }) {
  ...
  return (
    <div className={cn(
      ...
      entry.isMe ? 'bg-white/10 border-white/30 shadow-[0_0_15px_rgba(255,255,255,0.1)]' : 'bg-white/5 border-white/5 hover:bg-white/10',
    )}>
      {/* Position */}
      ...
      {/* Avatar */}
      <Avatar className="w-12 h-12 shrink-0 border-2 border-white/10 shadow-inner">
        <AvatarImage src={entry.avatarUrl || undefined} />
        <AvatarFallback className="text-sm font-bold bg-black text-white">{entry.name.slice(0, 2)}</AvatarFallback>
      </Avatar>

      {/* Name + rank */}
      <div className="flex-1 min-w-0">
        <p className={cn('text-base truncate mb-0.5', entry.isMe ? 'font-black text-white' : 'font-bold text-gray-200')}>
          {entry.name} {entry.isMe && <span className="text-gray-400 text-xs ml-1 uppercase tracking-wider">(You)</span>}
        </p>
      ...
      <div className="text-right shrink-0">
        <div className="flex items-center gap-1.5 justify-end mb-1">
          ...
          <span className="text-lg font-black text-white">{entry.xp.toLocaleString()}</span>
        </div>
        {change !== 0 && (
...
export default function LeaderboardPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* My position card */}
      {myEntry && (
        <div className="p-6 rounded-3xl border border-white/20 bg-white/10 backdrop-blur-2xl flex items-center gap-5 shadow-2xl relative overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/5 to-white/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
          <div className="w-16 h-16 rounded-2xl bg-white border border-white flex items-center justify-center text-black font-black text-2xl shadow-[0_0_20px_rgba(255,255,255,0.4)]">
            #{myEntry.rank}
          </div>
          <div className="relative z-10">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Your Position</p>
            <p className="font-display font-black text-2xl text-white">Rank #{myEntry.rank}</p>
          </div>
          <div className="ml-auto text-right relative z-10">
            ...
            <p className="font-black text-2xl text-white">{myEntry.xp.toLocaleString()}</p>
          </div>
        </div>
      )}

      <Tabs defaultValue="weekly" className="w-full">
        <TabsList className="w-full sm:w-auto grid grid-cols-3 bg-white/5 border border-white/10 p-1 rounded-xl">
          <TabsTrigger value="weekly" className="rounded-lg data-[state=active]:bg-white data-[state=active]:text-black font-bold text-xs uppercase tracking-wider py-2">Weekly</TabsTrigger>
          <TabsTrigger value="monthly" className="rounded-lg data-[state=active]:bg-white data-[state=active]:text-black font-bold text-xs uppercase tracking-wider py-2">Monthly</TabsTrigger>
          <TabsTrigger value="alltime" className="rounded-lg data-[state=active]:bg-white data-[state=active]:text-black font-bold text-xs uppercase tracking-wider py-2">All Time</TabsTrigger>
        </TabsList>
